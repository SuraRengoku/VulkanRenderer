...
class Application {
   ...
   private:
	...
	std::vector<Vertex> vertices = {};	 // wait to check
	std::vector<uint32_t> indices = {};	 // wait to check
	...
	std::vector<const char *> getRequiredExtensions(VkPhysicalDevice device) {
		...
		for (const auto &property : deviceExtensionProperties) {
			if (strcmp(property.extensionName, "VK_KHR_portability_subset") ==
				0) {
	 ...
	 * message VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT -> informational
	 * message VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT -> not
	 * necessarily an error, very likely a bug
	 * VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT -> invalid and may cause
	 * crashes
	 * @param messageType:
	...
	// VK_PRESENT_MODE_MAILBOX_KHR -> directly replace graphics in the queue
	// with submitted graphics instead of blocking application when the queue is
	// full
	...
	void createSwapChain() {
		...
		createInfo.imageArrayLayers =
		  1;  // for VR this variable may be more than 1
	...
	void createRenderPass() {
		...
		/*
		 ...
		 VK_ATTACHMENT_LOAD_OP_CLEAR -> use a constant value to clear the
		 attachment VK_ATTACHMENT_LOAD_OP_DONT_CARE -> continue drawing
		 */
		colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
		/*
		 VK_ATTACHMENT_STORE_OP_STORE -> the drawing result will be stored for
		 later reading VK_ATTACHMENT_STORE_OP_DONT_CARE -> the drawing result
		 won't be read
		 */
		...
		colorAttachment.finalLayout =
		  VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;	 // msaa attachment cannot
	...
	void createColorResources() {
		...
		// vulkan requirements: color attachment with multiple mass samples can
		// only have 1 mip level
	...
	void createTextureImage(const std::string &filename) {
		...
		stbi_uc *pixels =
		  stbi_load((__TEXTURE_PATH__ + filename).c_str(), &texWidth,
	...
	void generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth,
		...
		for (uint32_t i = 1; i < mips; ++i) {
			...
			vkCmdBlitImage(
			  cmdBuffer, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, image,
	...
	void createImage(uint32_t width, uint32_t height, uint32_t mips,
		...
		/*
		 * VK_IMAGE_LAYOUT_UNDEFINED: inaccessible for GPU, drop the texture
		 * after first transformation VK_IMAGE_LAYOUT_PREINITIALIZED:
		 * inaccessible for GPU, hold the texture after first transformation
		 */
		...
		if (vkAllocateMemory(device, &memoryAllocateInfo, nullptr,
	...
	void copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width,
		...
		/**
		 * @param bufferOffset the offset of the resource to be copied in the
		 * buffer
		 * @param bufferRowLength the way how the resource is stored in the
		 * buffer
		 * @param bufferImageHeight the way how the resource is stored in the
		 * buffer
		 */
	...
	void createGraphicsPipeline() {
		...
		rasterizer.depthClampEnable =
		  VK_FALSE;	 // fragments outside far plane and near plane will be
					 // clamped on the corresponding plane if depthClampEnable
					 // is VK_TRUE
		...
		// define the shader usage and uniform variables which can be
		// dynamically altered while rendering
		...
		/*
		 * @param device
		 * @param VkPipelineCache -> used to store graphics pipeline to
		 * accelerate creating pipelines later
		 */
	...
	void createCommandBuffers() {
		...
		/*
		 VK_COMMAND_BUFFER_LEVEL_PRIMARY -> can be submitted into queue to
		 execute but can not be called VK_COMMAND_BUFFER_LEVEL_SECONDARY -> can
		 not be directly submitted into queue but can be called to execute
		 */
	...
	void updateUniformBuffer(uint32_t curImageIdx) {
		...
		//    std::endl; std::cout << "View Matrix: " <<
		//    glm::to_string(ubo.view) << std::endl; std::cout << "Projection
		//    Matrix: " << glm::to_string(ubo.projection) << std::endl;
	...
	VkShaderModule createShaderModule(const std::vector<char> &code) {
		...
		// reinterpret_cast can convert a pointer type into other types, while
		// not changing the binary present
	...
	// TODO: should main loop also in the VulkanModule?
	void mainLoop() {
