...
VkResult VulkanModule::CreateDebugUtilsMessengerEXT(
...
void VulkanModule::DestroyDebugUtilsMessengerEXT(
...
std::vector<char> VulkanModule::readFile(const std::string &filename) {
	...
	if (!file.is_open()) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::readFile: failed to open file!");
	}
...
void VulkanModule::initWindow() {
	glfwInit();
	glfwWindowHint(GLFW_CLIENT_API,
				   GLFW_NO_API);  // do not create an OpenGL context
	glfwWindowHint(GLFW_RESIZABLE, GLFW_TRUE);	// allow to resize the window
...
void VulkanModule::framebufferResizedCallback(GLFWwindow *window, int width,
											  int height) {
	auto app =
	  reinterpret_cast<VulkanModule *>(glfwGetWindowUserPointer(window));
...
void VulkanModule::createInstance() {
	if (enableValidationLayers && !checkValidationLayerSupport()) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createInstance: validation layers "
	...
	if (vkCreateInstance(&createInfo, nullptr, &instance) != VK_SUCCESS)
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createInstance: failed to create "
		  "instance!");
...
void VulkanModule::createSurface() {
	if (glfwCreateWindowSurface(instance, window, nullptr, &surface) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createSurface: failed to create "
		  "window surface");
	}
...
bool VulkanModule::checkDeviceExtensionSupport(VkPhysicalDevice device) {
	...
	// check if all required extensions are fulfilled
}
...
std::vector<const char *> VulkanModule::getRequiredExtensions() const {
...
VkSampleCountFlagBits VulkanModule::getMaxUsableSampleCount() {
 ...
 *
 */
VKAPI_ATTR VkBool32 VKAPI_CALL VulkanModule::debugCallback(
...
void VulkanModule::populateDebugMessengerCreateInfo(
...
void VulkanModule::setupDebugMessenger() {
	...
	if (CreateDebugUtilsMessengerEXT(instance, &createInfo, nullptr,
									 &callback) != VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::setupDebugMessenger: failed to set up "
		  "debug callback!");
	}
...
void VulkanModule::pickPhysicalDevice() {
	...
	if (deviceCount == 0) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::pickPhysicalDevice: failed to find "
		  "GPUs with Vulkan support");
	}
	...
	if (candidates.rbegin()->first > 0) {
		...
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::pickPhysicalDevice: failed to find a "
		  "suitable GPU!");
	}
...
QueueFamilyIndices VulkanModule::findQueueFamilies(VkPhysicalDevice device) {
	...
	for (int i = 0; i < queueFamilyCount; ++i) {
		...
		// check if the i-queue in device support the surface, which is
		// important to make sure before setting swap chain
...
SwapChainSupportDetails VulkanModule::querySwapChainSupport(
  VkPhysicalDevice device) {
...
VkSurfaceFormatKHR VulkanModule::chooseSwapSurfaceFormat(
 ...
 */
VkPresentModeKHR VulkanModule::chooseSwapPresentMode(
...
VkExtent2D VulkanModule::chooseSwapExtent(
...
void VulkanModule::createLogicalDevice() {
	...
	if (vkCreateDevice(physicalDevice, &createInfo, nullptr, &device) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createLogicalDevice: failed to create "
		  "logical device!");
	}
...
void VulkanModule::createSwapChain() {
	...
	if (indices.graphicsFamily != indices.presentFamily) {
		createInfo.imageSharingMode =
		  VK_SHARING_MODE_CONCURRENT;  // the image can be used between
									   // multiple family
	...
	if (vkCreateSwapchainKHR(device, &createInfo, nullptr, &swapChain) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createSwapChain: failed to create "
		  "swap chain!");
	}
...
void VulkanModule::createRenderPass() {
	...
	if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &renderPass) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createRenderPass: failed to create "
		  "render pass!");
	}
...
VkFormat VulkanModule::findSupportedFormat(
  ...
  VkFormatFeatureFlags features) {
	...
	throw std::runtime_error(
	  "Vulkan_Support::VulkanModule::findSupportedFormat: failed to find a "
	  "supported format!");
}
...
void VulkanModule::createTextureImage(const std::string &filename) {
	...
	if (!pixels) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createTextureImage: failed to load "
		  "texture image!");
	}
...
void VulkanModule::generateMipmaps(VkImage image, VkFormat imageFormat,
	...
	if (!(formatProperties.optimalTilingFeatures &
		...
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::generateMipmaps: texture image format "
...
void VulkanModule::createImage(uint32_t width, uint32_t height, uint32_t mips,
	...
	if (vkCreateImage(device, &imageCreateInfo, nullptr, &image) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createImage: failed to create image!");
	}
	...
	if (vkAllocateMemory(device, &memoryAllocateInfo, nullptr, &imageMemory) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createImage: failed to allocate image "
		  "memory");
	}
...
void VulkanModule::transitionImageLayout(VkImage image, VkFormat format,
	...
	if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED &&
		...
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::transitionImageLayout: unsupported "
		  "layout transition!");
	}
...
void VulkanModule::copyBufferToImage(VkBuffer buffer, VkImage image,
...
VkImageView VulkanModule::createImageView(VkImage image, VkFormat format,
	...
	if (vkCreateImageView(device, &imageViewCreateInfo, nullptr, &imageView) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createImageView: failed to create "
		  "image view!");
	}
	return imageView;
}
...
void VulkanModule::createTextureSampler() {
	...
	if (vkCreateSampler(device, &samplerInfo, nullptr, &textureSampler) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createTextureSampler: failed to "
...
void VulkanModule::endSingleTimeCommands(VkCommandBuffer cmdBuffer) {
...
void VulkanModule::loadModel(const std::string &filename) {
...
void VulkanModule::createDescriptorPool() {
	...
	if (vkCreateDescriptorPool(device, &poolCreateInfo, nullptr,
		...
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createDescriptorPool: failed to "
...
void VulkanModule::createDescriptorSets() {
	...
	if (vkAllocateDescriptorSets(device, &setAllocateInfo,
		...
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createDescriptorSets: failed to "
 ...
 */
uint32_t VulkanModule::findMemoryType(uint32_t typeFilter,
	...
	throw std::runtime_error(
	  "Vulkan_Support::VulkanModule::findMemoryType: failed to find suitable "
	  "memory type!");
}
...
void VulkanModule::createBuffer(VkDeviceSize size,
	...
	if (vkCreateBuffer(device, &bufferCreateInfo, nullptr, &buffer) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createBuffer: failed to create "
		  "buffer!");
	}
	...
	if (vkAllocateMemory(device, &memoryAllocateInfo, nullptr, &deviceMemory) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createBuffer: failed to allocate "
		  "memory!");
	}
...
void VulkanModule::createDescriptorSetLayout() {
	...
	if (vkCreateDescriptorSetLayout(device, &layoutCreateInfo, nullptr,
		...
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createDescriptorSetLayout: failed to "
...
void VulkanModule::createGraphicsPipeline() {
	...
	if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr,
		...
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createGraphicsPipeline: failed to "
...
void VulkanModule::createFrameBuffers() {
	...
	for (size_t i = 0; i < swapChainImageViews.size(); ++i) {
		...
		if (vkCreateFramebuffer(device, &framebufferInfo, nullptr,
			...
			throw std::runtime_error(
			  "Vulkan_Support::VulkanModule::createFrameBuffers: failed to "
			  "create framebuffer!");
		}
	}
...
void VulkanModule::createCommandPool() {
	...
	if (vkCreateCommandPool(device, &poolInfo, nullptr, &commandPool) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createCommandPool: failed to create "
		  "command pool!");
	}
...
void VulkanModule::createCommandBuffers() {
	...
	if (vkAllocateCommandBuffers(device, &allocInfo, commandBuffers.data()) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createCommandBuffers: failed to "
...
void VulkanModule::recordCommandBuffer(VkCommandBuffer commandBuffer,
	...
	if (vkBeginCommandBuffer(commandBuffer, &beginInfo) != VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::recordCommandBuffer: failed to begin "
	...
	// enter the render pass
	{
		 ...
		 * VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS -> auxiliary command
		 * buffer exist
		 */
	...
	if (vkEndCommandBuffer(commandBuffer) != VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::recordCommandBuffer: failed to record "
		  "command buffer!");
	}
}

// deprecated, see createSyncObjects
void VulkanModule::createSemaphores() {
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
		if (vkCreateSemaphore(device, &semaphoreInfo, nullptr,
			...
			throw std::runtime_error(
			  "Vulkan_Support::VulkanModule::createSemaphores: failed to "
			  "create semaphore!");
		}
	}
}

void VulkanModule::createSyncObjects() {
	imageAvailableSemaphores.resize(MAX_FRAMES_IN_FLIGHT);
	renderFinishedSemaphores.resize(MAX_FRAMES_IN_FLIGHT);
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
		if (vkCreateSemaphore(device, &semaphoreInfo, nullptr,
			...
			vkCreateFence(device, &fenceInfo, nullptr, &inFlightFences[i]) !=
			  VK_SUCCESS) {
			throw std::runtime_error(
			  "Vulkan_Support::VulkanModule::createSyncObjects: failed to "
...
void VulkanModule::drawFrame() {
	...
	if (result == VK_ERROR_OUT_OF_DATE_KHR) {
		...
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::drawFrame: failed to acquire swap "
		  "chain image!");
	}
	...
	if (vkQueueSubmit(graphicsQueue, 1, &submitInfo,
		...
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::drawFrame: failed to submit draw "
		  "command buffer!");
	}
	...
	if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR ||
		...
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::drawFrame: failed to present swap "
		  "chain image!");
	}
...
VkShaderModule VulkanModule::createShaderModule(const std::vector<char> &code) {
	...
	if (vkCreateShaderModule(device, &createInfo, nullptr, &shaderModule) !=
		VK_SUCCESS) {
		throw std::runtime_error(
		  "Vulkan_Support::VulkanModule::createShaderModule: failed to create "
		  "shader module!");
	}
...
void VulkanModule::waitDeviceIdle() { vkDeviceWaitIdle(device); }
