...
void VulkanModule::waitDeviceIdle() {
    vkDeviceWaitIdle(device);
}
...
std::vector<const char *> VulkanModule::getRequiredExtensions(VkPhysicalDevice device) {
...
VkFormat VulkanModule::findSupportedFormat(const std::vector<VkFormat> &candidates,
...
void VulkanModule::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width,
                         uint32_t height) {
    VkCommandBuffer cmdBuffer = beginSingleTimeCommands();
...
void VulkanModule::createBuffer(VkDeviceSize size, VkBufferUsageFlags usageFlags,
...
void VulkanModule::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth,
...
VKAPI_ATTR VkBool32 VKAPI_CALL
    VulkanModule::debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
