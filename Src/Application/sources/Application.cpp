...
void Application::runApplicaton() {
	...
	auto renderWidgetPtr = std::make_unique<RenderWidget>(
	  window, 0, 0, WIDTH, HEIGHT, true, true, WidgetStatus::ACTIVATED);
	...
	auto fileWidgetPtr = std::make_unique<FileWidget>(
	  window, 0, 0, 300, HEIGHT, true, true, WidgetStatus::ACTIVATED);
	...
	auto propertyWidgetPtr = std::make_unique<PropertyWidget>(
	  window, WIDTH - 300, 0, 300, HEIGHT, true, true, WidgetStatus::ACTIVATED);
...
void Application::initImGui() {
	...
	init_info.DescriptorPool =
	  imguiDescriptorPool;	// 使用专用的descriptor pool
	...
	init_info.MSAASamples =
	  vulkanModule->getMSAASamples();  // 使用与主渲染管线相同的MSAA设置
...
void Application::renderPerformanceOverlay() {
	...
	if (ImGui::Begin("Performance", nullptr,
		...
		// 可以添加更多性能信息
		if (renderWidget) {
			ImGui::Text("Render Mode: %s", renderWidget->getWireframeMode()
											 ? "Wireframe"
											 : "Solid");
